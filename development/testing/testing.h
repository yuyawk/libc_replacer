  ...
  do {                                                                                                           \
    if (!(expr)) {                                                                                               \
      int discarded = /* NOLINTNEXTLINE(clang-analyzer-security.insecureAPI.DeprecatedOrUnsafeBufferHandling) */ \
          fprintf(                                                                                               \
              stderr,                                                                                            \
              "(TEST_ASSERT_TRUE) Assertion failed: %s is false, function "                                      \
              "%s, file %s, line %d.\n",                                                                         \
  ...
  do {                                                                                                           \
    if ((expr)) {                                                                                                \
      int discarded = /* NOLINTNEXTLINE(clang-analyzer-security.insecureAPI.DeprecatedOrUnsafeBufferHandling) */ \
          fprintf(                                                                                               \
              stderr,                                                                                            \
              "(TEST_ASSERT_FALSE) Assertion failed: %s is true, function "                                      \
              "%s, file %s, line %d.\n",                                                                         \
              #expr, __func__, __FILE__, __LINE__);                                                              \
      (void)discarded;                                                                                           \
      abort();                                                                                                   \
...
#define TEST_ASSERT_EQ(lhs, rhs)                                                                                 \
  do {                                                                                                           \
    if ((lhs) != (rhs)) {                                                                                        \
      int discarded = /* NOLINTNEXTLINE(clang-analyzer-security.insecureAPI.DeprecatedOrUnsafeBufferHandling) */ \
          fprintf(stderr,                                                                                        \
                  "(TEST_ASSERT_EQ) Assertion failed: %s == %s, function %s, "                                   \
                  "file %s, line %d.\n",                                                                         \
...
#define TEST_ASSERT_NE(lhs, rhs)                                                                                 \
  do {                                                                                                           \
    if ((lhs) == (rhs)) {                                                                                        \
      int discarded = /* NOLINTNEXTLINE(clang-analyzer-security.insecureAPI.DeprecatedOrUnsafeBufferHandling) */ \
          fprintf(stderr,                                                                                        \
                  "(TEST_ASSERT_NE) Assertion failed: %s != %s, function %s, "                                   \
                  "file %s, line %d.\n",                                                                         \
                  #lhs, #rhs, __func__, __FILE__, __LINE__);                                                     \
      (void)discarded;                                                                                           \
      abort();                                                                                                   \
...
/// @brief Implementation of @c TEST_ASSERT_TOKEN_EQ and
/// @c TEST_ASSERT_TOKEN_NE.
/// @param lhs Left-hand side.
/// @param rhs Right-hand side.
/// @param expected_equal 1 if the two are expected to equal, 0 otherwise
#define TEST_ASSERT_TOKEN_EQ_IMPLEMENTATION(lhs, rhs, expected_equal)                                              \
  do {                                                                                                             \
    if ((strcmp(#lhs, #rhs) == 0) != (expected_equal)) {                                                           \
      if (expected_equal) {                                                                                        \
        int discarded = /* NOLINTNEXTLINE(clang-analyzer-security.insecureAPI.DeprecatedOrUnsafeBufferHandling) */ \
            fprintf(stderr,                                                                                        \
                    "(TEST_ASSERT_TOKEN_EQ) Assertion failed: %s "                                                 \
        ...
        int discarded = /* NOLINTNEXTLINE(clang-analyzer-security.insecureAPI.DeprecatedOrUnsafeBufferHandling) */ \
            fprintf(stderr,                                                                                        \
                    "(TEST_ASSERT_TOKEN_NE) Assertion failed: %s "                                                 \
                    "!= %s, function %s, file %s, line %d.\n",                                                     \
                    #lhs, #rhs, __func__, __FILE__, __LINE__);                                                     \
        (void)discarded;                                                                                           \
      }                                                                                                            \
...
/// @param lhs Left-hand side.
/// @param rhs Right-hand side.
#define TEST_ASSERT_TOKEN_EQ(lhs, rhs)                                         \
  TEST_ASSERT_TOKEN_EQ_IMPLEMENTATION(lhs, rhs, 1)

/// @brief Abort if @c lhs is an equal compile-time token to @c rhs.
/// @param lhs Left-hand side.
/// @param rhs Right-hand side.
#define TEST_ASSERT_TOKEN_NE(lhs, rhs)                                         \
  TEST_ASSERT_TOKEN_EQ_IMPLEMENTATION(lhs, rhs, 0)
